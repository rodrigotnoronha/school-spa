...
const bypassHeader = url => url.indexOf('auth/login') !== -1 || url.indexOf('/auth/refresh') !== -1 || url.indexOf('/auth/password/email') !== -1 || url.indexOf('/auth/password/reset') !== -1
const bypassSetup = url => url.indexOf('auth/user') !== -1
const getToken = store => get(store, 'getters.jwtToken', null)
const getHeader = (request, store) => {
  const token = getToken(store)
  return isEmpty(token) ? null : `Bearer ${getToken(store)}`
}
